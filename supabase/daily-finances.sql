...
alter table public.daily_finance_sales
add column if not exists transaction_group_id uuid;

update public.daily_finance_sales
...
create index if not exists daily_finance_sales_transaction_group_idx
on public.daily_finance_sales (transaction_group_id, created_at desc);
